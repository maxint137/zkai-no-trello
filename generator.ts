...
const data: { [key: string]: Board } = {
    Adam: {
        ...
        assignmentsDefinition: [
            { subject: 'Math 🧮', count: 7, rounds: 1, labels: ['Math', 'Class'], steps: ClassSteps },
            { subject: 'Japanese 🇯🇵', count: 7, rounds: 1, labels: ['Jap', 'Class'], steps: ClassSteps },
...
async function setupStaticData(boards: { [key: string]: Board }) {
    ...
    await Promise.all(
        Object.keys(boards).map(async (name) => {
            ...
            board.boardId = (await trelloApi('/members/me/boards'))
                .filter((b: { name: string }) => b.name === board.boardName)[0]?.id;
            if (!board.boardId) {
                throw new Error(`No board named ${board.boardName} found`);
            }

            board.todoListId = (await trelloApi(`boards/${board.boardId}/lists`))
                .filter((list: { name: string }) => list.name.startsWith('To Do'))[0]?.id;
            if (!board.todoListId) {
                throw new Error(`No todo list found`);
            }

            const labels = (await trelloApi(`boards/${board.boardId}/labels`))
            ...
            [...board.assignmentsDefinition, ...board.drillsDefinition].forEach((assignment) => {
                assignment.labels?.forEach((label) => allLabels.add(label));
            });

            board.labelIds = Array.from(allLabels).reduce((acc: { [key: string]: string }, label) => {
                const labelId = labels.filter((l: { name: string }) => l.name === label)[0]?.id;
...
function calculateTaskDates(
    ...
    const endDate = new Date(startDate.getFullYear(), startDate.getMonth(), daysInMonth);
    ...
    const spinTasks = (
        ...
        for (let i = 0; i < assignment.count; i++) {
            ...
            currentDate = new Date(startDate.getTime() + daysPerAssignment * (1000 * 60 * 60 * 24));

            const index = hasExam && i == assignment.count - 1 ? '🧐' : toEmojiDigit(i + 1);
...
async function _trelloApi(endpoint: string, body: any = null): Promise<any> {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(`${url} got an HTTP error ${response.status}: ${errorText}`);
        }
...
async function trelloCardCreate(
    ...
    console.log(`Creating card for ${card.subject} ${card.startDate.toLocaleDateString()} -> ${card.dueDate.toLocaleDateString()}`);
...
setupStaticData(data).then(() => {
    ...
    Object.keys(data).forEach((name) => {
        const { assignmentsDefinition, drillsDefinition, userId, todoListId, labelIds } = data[name];
            ...
            delay(index * 1000)
                ...
                .then((newCard) =>
                    trelloApi(`cards/${newCard.id}/idMembers`, { "value": userId })// add the user
                        .then(() => { trelloAddChecklist(newCard.id, card.prototype?.steps) }) // add the checklist
                        .then(() => {
                            trelloAddLabels(newCard.id,
